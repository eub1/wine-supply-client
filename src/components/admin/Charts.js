...
function Charts() {
	const StatsUserWeek = async () => {
		let result = await axios.get(
			`https://wine-supply-api-production.up.railway.app/admin/stats/perweek?model=UserModel`,
			{
				headers: {
	...
	const StatsShoppingWeek = async () => {
		let result = await axios.get(
			`https://wine-supply-api-production.up.railway.app/admin/stats/perweek?model=ShoppingOrder`,
			{
				headers: {
					Authorization: `Bearer ${JSON.parse(token)}`,
					"Content-Type": "application/json, charset=utf-8",
	...
	const ZonesPie = async () => {
		let result = await axios.get(
			"https://wine-supply-api-production.up.railway.app/admin/stats/wines/fields?origin=1",
			{
				headers: {
					Authorization: `Bearer ${JSON.parse(token)}`,
					"Content-Type": "application/json, charset=utf-8",
	...
	//   let result = await axios.get(`https://wine-supply-api-production.up.railway.app/admin/stats/permonth?model=ShoppingOrder`, {
	//     headers: {
	//       Authorization: `Bearer ${JSON.parse(token)}`,
	//       "Content-Type": "application/json, charset=utf-8",
	//     },
